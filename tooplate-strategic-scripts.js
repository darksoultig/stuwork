...
serviceTabs.forEach(tab => {
   tab.addEventListener('click', function () {
      ...
      // Attempt to find a container, fallback to document if not found (though structure implies container)
      const container = this.closest('.services-container') || document;

      // Remove active class from tabs and details WITHIN this container only
      const containerTabs = container.querySelectorAll('.service-tab');
      const containerDetails = container.querySelectorAll('.service-details');

      containerTabs.forEach(t => t.classList.remove('active'));
      containerDetails.forEach(d => d.classList.remove('active'));

      // Add active class to clicked tab and corresponding detail
      this.classList.add('active');
      const detail = container.querySelector(`.service-details[data-service="${target}"]`);
